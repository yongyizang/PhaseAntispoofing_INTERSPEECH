...
def calculate_tDCF_EER(cm_scores_file,
    ...
    if printout:
        ...
        # also write output in panda format
        arrayToOutput = [eer_cm * 100, min_tDCF]
        for attack_type in attack_types:
            arrayToOutput.append(eer_cm_breakdown[attack_type] * 100)
        pd.DataFrame(arrayToOutput).to_csv(f"{output_file}.csv", index=False)
